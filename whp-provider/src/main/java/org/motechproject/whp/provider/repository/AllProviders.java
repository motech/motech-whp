 ...
 */
@Repository
public class AllProviders extends MotechBaseRepository<WHPProvider> {

    @Autowired
    public AllProviders(@Qualifier("whpDbConnector") CouchDbConnector dbCouchDbConnector) {
        super(WHPProvider.class, dbCouchDbConnector);
    }

    @GenerateView
    public WHPProvider findByProviderId(String providerId) {
        ViewQuery find_by_providerId = createQuery("by_providerId").key(providerId).includeDocs(true);
        return singleResult(db.queryView(find_by_providerId, WHPProvider.class));
    }
}
